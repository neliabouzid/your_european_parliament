...
# 1. On charge les variables AVANT TOUT LE RESTE
load_dotenv() 
...
@app.route('/positions/<path:reference>.html')
def show_position(reference):
    ...
    cur.execute("""
        SELECT reference, title, "Stage reached in procedure" AS stage, 
                subjects, final_act_summary, key_players, key_events
        FROM for_mvp_final_act
        WHERE reference = %s
    """, (reference,))
    ...
    position = {
        ...
        'final_act_summary': row['final_act_summary'] or "Aucun résumé disponible",
    }
